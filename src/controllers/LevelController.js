...
class LevelController {
  async findAll(req, res) {
    try {
      const level = await LevelService.findAll({}, "", "cefrId");
    ...
    try {
      ...
      if (existKey) {
        return res
          ...
          .json({
            status: false,
            message: "Bu Sıralamada Zaten Bir Kayıt Bulunmaktadır",
          });
      }
      const existCefrKey = await CEFRServices.findById(req.body.cefrId);
      if (!existCefrKey) {
        return res
          .status(200)
          .json({
            status: false,
            message: "Bu CEFR Anahtarında Kayıt Bulunamadı",
          });
      }
  ...
  async update(req, res) {
    try {
      const existKey = await LevelService.findOne({ key: req.body.key });
      if (existKey._id.toString() !== req.params.id) {
        return res
          .status(200)
          .json({
            status: false,
  ...
  async delete(req, res) {
    try {
      const existContent = await ContentService.findOne({
      ...
      if (existContent) {
        return res
          ...
          .json({
            status: false,
            message: "Bu Sıralamaya Ait İçerik Bulunmaktadır",
          });
      }
      const level = await LevelService.deleteById(req.params.id);
      return res
        .status(200)
        .json({ status: true, message: "Sıralama Başarıyla Silindi", level });
    } catch (error) {
      return res.status(500).json({ error: error.message });
    }
  }
